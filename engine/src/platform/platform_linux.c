...
b8 platform_startup(platform_state *plat_state, const char *appication_name,
    ...
    if (xcb_connection_has_error(state->connection)) {
        VFATEL("Failed to connect to X server via XCB");
        return FALSE;
    }
    ...
    // Create the window(also called a cookie)
    ...
    xcb_change_property(state->connection, XCB_PROP_MODE_REPLACE, state->window,
                        ...
                        8, // How many bits to read at a time
                        strlen(appication_name), appication_name);
    ...
    if (stream_result <= 0) {
        VFATEL("An error occurred when flushing the stream: %d", stream_result);
        return FALSE;
    }
...
b8 platform_pump_messages(platform_state *plat_state) {
    ...
    while (event != 0) {
        ...
        case XCB_KEY_RELEASE: {
            ...
            // Translate linux specific keycode to windows based keycodes
...
f64 platform_get_absolute_time() {
    ...
    clock_gettime(
        CLOCK_MONOTONIC,
        &now); // THIS error is a fake it is real and the program will compile
