...
void *_darray_push(void *array, const void *value_ptr) {
    ...
    u64 addr = (u64)array;     // create pointer to insert new element
    addr += (length * stride); // move pointer to end of array
...
void *_darray_pop_at(void *array, u64 index, void *dest) {
    ...
    u64 stride = darray_stride(array);
    if (index >= length) {
        VERROR("Index outside of array! Length: %llu, index: %llu", length,
               index);
        return array;
    }
    ...
    if (index != length - 1) {
        vcopy_memory((void *)(addr + (index * stride)),
...
void *_darray_insert_at(void *array, u64 index, void *value_ptr) {
    ...
    u64 stride = darray_stride(array);
    if (index >= length) {
        VERROR("Index outside of array! Length: %i, index: %i", length, index);
        return array;
    }
    if (length >= darray_capacity(array)) {
        array = _darray_resize(array);
    ...
    if (index != length - 1) {
        vcopy_memory((void *)(addr + ((index + 1) * stride)),
